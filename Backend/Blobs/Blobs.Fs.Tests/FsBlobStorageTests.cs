...
namespace Blobs.Fs.Tests;

public sealed class TestsFixture : IDisposable
{
    internal readonly FsBlobStorage Storage;
    private readonly string _rootDirPath = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
    ...
    public TestsFixture()
    {
        ...
        options
            .Setup(x => x.Value)
            .Returns(new FsBlobStorage.Options
            {
...
public sealed class FsBlobStorageTests : IClassFixture<TestsFixture>
{
    ...
    public async Task OpenRead_NotExistedBlob()
    {
        ...
        // Assert
        await Assert.ThrowsAsync<CodedException<IBlobStorage.ErrorCodes>>(() => blobStorage.OpenRead(blobId));
    }
    ...
    public async Task OpenWrite_BeforeCreate()
    {
        ...
        // Assert
        await Assert.ThrowsAsync<CodedException<IBlobStorage.ErrorCodes>>(() => blobStorage.OpenReadWrite(blobId));
    }
    ...
    public async Task Remove_NotExistedBlob()
    {
        ...
        // Assert
        await Assert.ThrowsAsync<CodedException<IBlobStorage.ErrorCodes>>(() => blobStorage.Remove(blobId));
    }
