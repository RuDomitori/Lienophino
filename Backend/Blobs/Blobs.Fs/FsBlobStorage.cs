...
public sealed class FsBlobStorage : BaseBlobStorage
{
    ...
    /// <inheritdoc/>
    public override Task<Stream> OpenRead(Guid blobId, CancellationToken cancellationToken = default)
    {
        if (blobId == default)
        ...
        try
        {
            ...
            if (!fileInfo.Exists)
                throw new CodedException<IBlobStorage.ErrorCodes>(
                    "The BLOB not found", IBlobStorage.ErrorCodes.BlobNotFound
                );
        ...
        {
            throw new CodedException<IBlobStorage.ErrorCodes>("Internal error", IBlobStorage.ErrorCodes.Internal, e);
        }
    }
    
    /// <inheritdoc/>
    public override Task<Stream> OpenReadWrite(Guid blobId, CancellationToken cancellationToken = default)
    {
        if (blobId == default)
            throw new ArgumentException($"{nameof(blobId)} must be not default", nameof(blobId));
    ...
    public override Task<Stream> Create(Guid blobId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (fileInfo.Exists)
                throw new CodedException<IBlobStorage.ErrorCodes>(
                    "The BLOB already exists", IBlobStorage.ErrorCodes.BlobAlreadyExists
                );
    ...
    /// <inheritdoc/>
    public override Task<bool> RemoveIfExist(Guid blobId, CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc/>
    public override Task Move(Guid oldBlobId, Guid newBlobId, CancellationToken cancellationToken = default)
    {
        if (oldBlobId == default)
            throw new ArgumentException($"{nameof(oldBlobId)} must be not default", nameof(oldBlobId));

        if (newBlobId == default)
            throw new ArgumentException($"{nameof(newBlobId)} must be not default", nameof(newBlobId));
        ...
        try
        {
            if (File.Exists(GetBlobFilePath(oldBlobId)) is false)
                throw new CodedException<IBlobStorage.ErrorCodes>(
                    "A blob with that id was not found", IBlobStorage.ErrorCodes.BlobNotFound
                );

            if (File.Exists(GetBlobFilePath(newBlobId)))
                throw new CodedException<IBlobStorage.ErrorCodes>(
                    "A blob with the same id already exists", IBlobStorage.ErrorCodes.BlobAlreadyExists
                );

            File.Move(
                GetBlobFilePath(oldBlobId),
                GetBlobFilePath(newBlobId)
            );
        ...
        catch (Exception e)
        {
            throw new CodedException<IBlobStorage.ErrorCodes>(
                ...
                IBlobStorage.ErrorCodes.Internal, e
            );
        }
    }
